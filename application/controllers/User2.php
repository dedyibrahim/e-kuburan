...
$h_berkas = $this->M_user2->hitung_berkas()->num_rows()+1;
...
$query = $this->M_user2->data_berkas('Masuk');
...
$query = $this->M_user2->data_berkas('Proses');
...
$data           = $this->M_user2->data_berkas_proses($this->uri->segment(3));    
...
if($this->input->post()){
$data      = $this->M_user2->data_form_perizinan($this->input->post('no_berkas'));
...
echo "<div class='col-md-4  mb-2' id='syarat".$form['id_syarat_dokumen']."'>"
."<div class='card'>"
...
."<button class='btn btn-sm btn-block  btn-danger btn_hapus_syarat".$form['id_syarat_dokumen']."' onclick='hapus_syarat(".$form['id_syarat_dokumen'].")'>Hapus syarat <i class='fa fa-trash'></i></button>"
."</div>"
."</div>"
."</div>";
}else{    
...
."<button class='btn btn-block  btn-sm btn-danger  btn_hapus_syarat".$form['id_syarat_dokumen']."' onclick='hapus_syarat(".$form['id_syarat_dokumen'].")'>Hapus syarat <i class='fa fa-trash'></i></button>"
."</div>"    
."</div>"    
."</div>";
...
$data = $this->M_user2->data_dokumen_utama($this->input->post('no_berkas'))->row_array();
...
if($this->input->post()){
$data = $this->M_user2->data_form_perorangan($this->input->post('no_berkas'));
...
$term = strtolower($this->input->get('term'));    
$query = $this->M_user2->cari_data_perorangan($term);
...
$this->M_user2->simpan_syarat($data);
...
if($this->input->post()){
$data = $this->input->post();
$h_identitas = $this->M_user2->data_perorangan()->num_rows()+1;
...
$this->M_user2->simpan_data_perorangan($data_perorangan);
...
$this->M_user2->simpan_syarat_perorangan($data2);    
...
if($this->input->post()){
$this->M_user2->hapus_data_syarat_perorangan($this->input->post('id_data_syarat_perorangan'));    
}else{
redirect(404);    
}
}
...
$this->M_user2->simpan_syarat_perorangan($data);    
$status = array(
"status"=>"Berhasil",
...
if($this->input->post()){
$query = $this->M_user2->ambil_data_syarat_perorangan($this->input->post('id_data_perorangan'))->row_array();
...
if($this->input->post()){
$this->M_user2->hapus_syarat_dokumen($this->input->post('id_syarat_dokumen'));    
}else{
redirect(404);    
}
}
